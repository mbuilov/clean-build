...
# CPU, TCPU - one of $(SUPPORTED_CPUS),
# UTILS     - one of $(SUPPORTED_UTILS),
# COMPILERS - set of $(SUPPORTED_COMPILERS)
...
# note: OS may be overridden either in command line or in project configuration makefile
ifndef OS
OS := LINUX
...
# CPU processor architecture we are building applications for (x86, armv5, etc.)
...
# note: TCPU may be overridden either in command line or in project configuration makefile
TCPU := $(CPU)

# UTILS - flavor of system utilities (such as cp, mv, rm, etc.)
# note: UTILS may be overridden either in command line or in project configuration makefile
ifeq (LINUX,$(OS))
UTILS := gnu
...
ifeq (,$(filter $(UTILS),$(SUPPORTED_UTILS)))
$(error unknown UTILS=$(UTILS), please pick flavour of system utilities: $(SUPPORTED_UTILS))
endif
...
ifndef OS
$(error OS undefined, please pick one of target OS types: $(SUPPORTED_OSES))
endif

ifeq (,$(filter $(OS),$(SUPPORTED_OSES)))
$(error unknown OS=$(OS), please pick one of target OS types: $(SUPPORTED_OSES))
endif
...
ifndef COMPILERS
$(error COMPILERS undefined, please select which compilers to use: $(SUPPORTED_COMPILERS))
endif

ifneq (,$(filter-out $(SUPPORTED_COMPILERS),$(COMPILERS)))
$(error unknown compiler(s): $(filter-out \
  $(SUPPORTED_COMPILERS),$(COMPILERS)), please use compilers from list: $(SUPPORTED_COMPILERS))
endif
...
# list of all processed target makefiles names
# note: PROCESSED_MAKEFILES is never cleared, only appended
PROCESSED_MAKEFILES:=

# remember processed makefile, used by DEF_HEAD_CODE
