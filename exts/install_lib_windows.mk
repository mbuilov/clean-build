...
# define windows-specific INSTALL_LIBS macro
...
# note: $(D_INCLUDEDIR), $(D_LIBDIR) _may_ return paths with spaces, but $($1_LIBRARY_HDIR) must not contain spaces
# $1 - library name (mylib for libmylib.a)
define DEFINE_INSTALL_LIB_VARIABLES_WIN
DST_INC_DIR := $(subst $(space),\ ,$(D_INCLUDEDIR))$($1_LIBRARY_HDIR)
DST_LIB_DIR := $(subst $(space),\ ,$(D_LIBDIR))
endef
...
# note: $(DEFINE_INSTALL_LIB_VARIABLES) and then $(DEFINE_INSTALL_LIB_VARIABLES_WIN) must be evaluated before expanding this macro
...
install_$1_headers:$(if $($1_LIBRARY_HEADERS),| $(DST_INC_DIR)$(newline)$(tab)$$(call \
  SUP,COPY,$$(call ospath,$$(HEADERS)) -> $$(call ifaddq,$$(call ospath,$$(D_INCLUDEDIR)$($1_LIBRARY_HDIR))),1,1)$$(foreach \
  h,$$(HEADERS),$$(newline)$(QUIET)$$(call CP,$$h,$$(call ifaddq,$$(D_INCLUDEDIR)$($1_LIBRARY_HDIR)))))

uninstall_$1_headers:$(if $($1_LIBRARY_HEADERS),$(newline)$(tab)$(if $($1_LIBRARY_HDIR),$$(call \
  SUP,RD,$$(call ifaddq,$$(call ospath,$$(D_INCLUDEDIR)$($1_LIBRARY_HDIR))),1,1)$$(call \
  DEL_DIR,$$(call ifaddq,$$(D_INCLUDEDIR)$($1_LIBRARY_HDIR))),$$(foreach h,$$(notdir $$(HEADERS)),$$(newline)$$(call \
  SUP,DEL,$$(call ifaddq,$$(call ospath,$$(D_INCLUDEDIR)/$$h)),1,1)$$(call DEL,$$(call ifaddq,$$(D_INCLUDEDIR)/$$h)))))
...
install_$1:$(if $($1_LIBRARY_NO_INSTALL_HEADERS),,install_$1_headers)$(if \
 ...
 SUP,INSTALL,$$(call ifaddq,$$(call ospath,$$(D_LIBDIR)/$$(notdir $$l))),1,1)$$(call CP,$$l,$$(call ifaddq,$$(D_LIBDIR))))$(if \
  ,)$(newline)$(tab)$$(foreach d,$$(BUILT_DLLS),$$(newline)$$(call \
 SUP,INSTALL,$$(call ifaddq,$$(call ospath,$$(D_LIBDIR)/$$(notdir $$d))),1,1)$$(call CP,$$d,$$(call ifaddq,$$(D_LIBDIR))))$(if \
  $($1_LIBRARY_NO_INSTALL_IMPS),,$(newline)$(tab)$$(foreach i,$$(BUILT_IMPS),$$(newline)$$(call \
 SUP,INSTALL,$$(call ifaddq,$$(call ospath,$$(D_LIBDIR)/$$(notdir $$i))),1,1)$$(call CP,$$i,$$(call ifaddq,$$(D_LIBDIR))))))
...
uninstall_$1:$(if $($1_LIBRARY_NO_INSTALL_HEADERS),,uninstall_$1_headers)$(if \
 ...
 SUP,DEL,$$(call ifaddq,$$(call ospath,$$(D_LIBDIR)/$$l)),1,1)$$(call DEL,$$(call ifaddq,$$(D_LIBDIR)/$$l)))$(if \
  ,)$(newline)$(tab)$$(foreach d,$$(notdir $$(BUILT_DLLS)),$$(newline)$$(call \
 SUP,DEL,$$(call ifaddq,$$(call ospath,$$(D_LIBDIR)/$$d)),1,1)$$(call DEL,$$(call ifaddq,$$(D_LIBDIR)/$$d)))$(if \
  $($1_LIBRARY_NO_INSTALL_IMPS),,$(newline)$(tab)$$(foreach i,$$(notdir $$(BUILT_IMPS)),$$(newline)$$(call \
 SUP,DEL,$$(call ifaddq,$$(call ospath,$$(D_LIBDIR)/$$i)),1,1)$$(call DEL,$$(call ifaddq,$$(D_LIBDIR)/$$i)))))
...
INSTALL_LIBS = $(eval \
  ...
  $(DEFINE_INSTALL_LIB_VARIABLES_WIN))$(eval \
  $(INSTALL_LIBS_TEMPLATE_WINDOWS))

# protect variables from modifications in target makefiles
$(call SET_GLOBAL,DEFINE_INSTALL_LIB_VARIABLES_WIN INSTALL_LIBS_TEMPLATE_WINDOWS INSTALL_LIBS)
