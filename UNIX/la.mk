...
INSTALLED_LA ?= '$(DESTDIR)$(LIBDIR)/$(LIBTOOL_LA_NAME)'
...
INSTALLED_LAS1 = $(foreach r,$1,$(foreach l,$(firstword $(subst ?, ,$r)),$(call INSTALLED_LA,$(filter $l,$2),$1)))
...
INSTALLED_LAS ?= $(call INSTALLED_LAS1,$1,$(patsubst \
...
# $1 - static library name (may be empty if $2 is not empty)
# $2 - dynamic library name (may be empty if $1 is not empty)
LA_GENERATOR ?= $(call LIBTOOL_LA_TEMPLATE,$1,$2,$(MODVER))

# install .la-file
# $1 - static library name (may be empty if $2 - non-empty)
# $2 - dynamic library name (may be empty if $1 - non-empty)
INSTALL_LA ?= $(call ECHO_INSTALL,$(LA_GENERATOR),$(INSTALLED_LA),755)

# $1 - all built libraries (result of $(GET_ALL_LIBS))
# $2 - names of built static libraries, may be empty
# $3 - built dynamic library name, may be empty
INSTALL_LAS1 = $(foreach r,$1,$(foreach l,$(firstword $(subst ?, ,$r)),$(newline)$(call INSTALL_LA,$(filter $l,$2),$3)))

# install .la-files
# $1 - all built libraries (result of $(GET_ALL_LIBS))
# $2 - paths to built static libraries, may be empty
# $3 - path to built dynamic library, may be empty
INSTALL_LAS ?= $(call INSTALL_LAS1,$1,$(patsubst \
  $(LIB_PREFIX)%$(LIB_SUFFIX),%,$(notdir $2)),$(patsubst \
  $(DLL_PREFIX)%$(DLL_SUFFIX),%,$(notdir $3)))

# protect variables from modifications in target makefiles
$(call CLEAN_BUILD_PROTECT_VARS,LIBTOOL_LA_NAME LIBTOOL_LA_TEMPLATE \
  INSTALLED_LA INSTALLED_LAS1 INSTALLED_LAS LA_GENERATOR INSTALL_LA INSTALL_LAS1 INSTALL_LAS)
