...
# note: filter-out $(project_exported_vars) - exported variables (e.g. command-line) should not be traced
...
$(eval cb_target_makefile += $$(call set_global,$(foreach =,$(filter-out $(dump_max) %.^e $(project_exported_vars),$(.VARIABLES)),$(if \
  $(findstring override,$(flavor $=)),$=)),project))
endif

ifdef cb_checking
# protect from changes $(project_exported_vars) (at least command-line variables), do not trace calls to them to not redefine the values
# note: 'cb_target_makefile' variable is used here temporary and will be redefined later
...
# note: reset 'cb_var_access_err' macro at last, because it's used in the code generated by $(cb_reset_local_var)
...
# note: reset 'temporary_overridden' variable - it may be set before $(cb_def_tail)
...
$(if $1,$$(if $$(cb_need_tail_code),$$(error \
  $$$$(define_targets) was not expanded at end of $$(cb_need_tail_code))),$(if \
  $(cb_need_tail_code),,$(error $$(cb_def_head) was not evaluated at head of makefile!)))$(foreach \
